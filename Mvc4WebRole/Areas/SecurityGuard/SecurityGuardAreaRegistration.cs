...
namespace Mvc4WebRole.Areas.SecurityGuard
{
    public class SecurityGuardAreaRegistration : AreaRegistration
    {
        ...
        public override void RegisterArea(AreaRegistrationContext context)
        {
            context.MapRoute("SearchMembership", "SecurityGuard/Membership/index/{searchterm}/{filterby}",
                new
            { controller = "Membership", action = "Index", searchterm = UrlParameter.Optional, filterby = "all" }
            );

            context.MapRoute("Membership", "SecurityGuard/Membership/{action}/{userName}",
                new
            { controller = "Membership", userName = UrlParameter.Optional }
            );
            ...
            context.MapRoute(
                ...
                new
            { controller = "Dashboard", action = "Index", id = UrlParameter.Optional }
            );
        }
    }
}
